...
Há três percursos, e a escolha é sua. A trilha completa, de 12 etapas, é a que sustenta a avaliação de 10 pontos. A Trilha Rápida, de sete passos em cerca de seis aulas, serve a turmas com carga reduzida, recuperação de percurso, feiras e revisão antes da apresentação. A Trilha Extrema é um sprint cronometrado de 50 minutos, em quatro blocos, que cabe em uma aula: o relógio avança de bloco em bloco sozinho e a equipe termina com uma frase de problema, três relatos, um papel testado por duas pessoas e um pitch de um minuto.
Use a Trilha Extrema para abrir o módulo, para uma oficina avulsa, para destravar uma equipe parada ou como aquecimento. Ela não substitui a investigação avaliada: três conversas e dois testes em uma aula não sustentam afirmações sobre o problema, o público ou a solução. A própria página avisa isso ao aluno, exige que o último bloco declare o que a equipe ainda não sabe, e repete o aviso no resumo exportado. Se receber um sprint como entrega de nota, peça a investigação completa antes de pontuar.'''),
