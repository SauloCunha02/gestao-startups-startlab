...
(async () => {
  ...
  try {
    ...
    assert.match(await box.textContent(), /duolingo/i, 'exemplo é o caso real do Duolingo');
    ...
    assert.doesNotMatch(await page.locator('main').textContent(), /Fila Menor/i, 'sem resquício do caso antigo');
