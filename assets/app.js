...
(() => {
  ...
  const quizzes = [
    ['O primeiro trabalho de uma startup é…', ['Criar o nome e o logotipo.', 'Escolher um território: pessoas e uma dificuldade que se repete.', 'Programar um aplicativo completo.'], 1, 'O iFood começou por um momento específico — pedir comida por telefone —, não por “alimentação”. Marca e tecnologia vêm depois.'],
    ['“As pessoas vão querer isso”, sem nenhuma verificação, é…', ['Uma certeza.', 'Uma conclusão de teste.', 'Uma suposição.'], 2, 'A Netflix enviou um disco pelo correio para si mesma antes de apostar no negócio. Suposição sem evidência continua suposição — e é a que derruba a startup depois.'],
    ['Qual pergunta ajuda a ouvir sem induzir?', ['Conte a última vez em que isso aconteceu.', 'Nossa ideia é ótima, não é?', 'Você assinaria este serviço incrível?'], 0, 'Um episódio real revela contexto, frequência e a alternativa atual. Os fundadores do Airbnb foram até Nova York justamente para ouvir isso de perto.'],
    ['Importância 4,8 e satisfação 1,6 sugerem…', ['Negócio garantido.', 'Uma dor que merece investigação.', 'Que ninguém se importa.'], 1, 'É uma pista forte para investigar, junto dos relatos. No caso do Spotify, a dor prioritária era a conveniência, não o preço.'],
    ['Definir o cliente como “todo mundo”…', ['Aumenta as chances de dar certo.', 'Facilita encontrar quem testar.', 'Deixa a equipe sem ninguém concreto para atender e testar.'], 2, 'A Nubank começou por um recorte estreito: quem já resolvia a vida pelo celular. Startups começam estreitas de propósito.'],
    ['Uma proposta de valor explica…', ['Para quem é, qual benefício e como ajuda.', 'Só o nome e a cor da marca.', 'Quantos slides serão feitos.'], 0, 'A do Duolingo cabe em uma frase e não usa “revolucionário”. Relacione o benefício com a dor escolhida; a promessa ainda precisa ser testada.'],
    ['No caso da 99, “ficar em casa e não sair” é…', ['Uma força interna da equipe.', 'Uma alternativa que o cliente já usa.', 'Algo que deve ser ignorado.'], 1, 'A mesma necessidade é atendida de formas diferentes — inclusive deixando de consumir. Se o cliente já resolve aquilo de algum jeito, concorrente existe.'],
    ['No Mercado Livre, quem compra não paga para usar o site. Isso mostra que…', ['O negócio não tem receita.', 'Nem sempre quem usa é quem paga.', 'O site não tem fins lucrativos.'], 1, 'A receita vem de comissão do vendedor, anúncios e serviços de pagamento. Separar usuário de pagador é o que faz o modelo se explicar.'],
    ['O vídeo do Dropbox, feito antes do produto ficar pronto, serviu para…', ['Substituir o produto para sempre.', 'Testar se havia interesse real, gastando pouco.', 'Treinar a equipe de vendas.'], 1, 'A primeira versão não precisa funcionar por completo: precisa poder ser experimentada e responder a uma pergunta importante.'],
    ['O critério de sucesso deve ser definido…', ['Depois de ver quem conseguiu.', 'Antes de começar o teste.', 'Só se os usuários reclamarem.'], 1, 'Definir antes evita mudar a meta para fazer o resultado parecer melhor. Depois do teste, o critério vira desculpa.'],
    ['O Instagram cortou quase todas as funções porque…', ['A equipe se cansou delas.', 'Os usuários só usavam a de fotos.', 'Era mais barato manter uma só.'], 1, 'Pivotar não é desistir: é seguir a evidência. Use os registros para priorizar a mudança e teste de novo.'],
    ['Um bom pitch distingue…', ['Evidências, estimativas e dúvidas.', 'Somente pontos positivos.', 'A equipe dos concorrentes por elogios.'], 0, 'Mostrar o que foi aprendido e o que falta investigar torna a startup compreensível. Quem esconde a dúvida perde credibilidade.']
  ];
  ...
  function exampleBlock(key,example,headers) {
    ...
    return `<p class="example-row"><button class="ex-toggle" data-example="${key}" aria-expanded="${showExamples}" aria-controls="ex-${key}">${icon('bulb')} <span>${showExamples?'Ocultar exemplo':'Ver exemplo preenchido'}</span></button></p><div class="example-box" id="ex-${key}" ${showExamples?'':'hidden'}><span class="example-tag">EXEMPLO REAL · AIRBNB (2007–2009)</span>${body}<p class="example-warn">Referência de raciocínio, não de tamanho — e não resposta pronta. A história da empresa é pública; entrevistas, notas e tempos de teste são ilustrativos, para mostrar o formato esperado. Copiar o exemplo não gera evidência: escreva com os registros da sua equipe.</p></div>`;
  }
