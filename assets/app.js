...
(() => {
  ...
  const defaults = () => ({schema:2, team:'', completed:[], answers:{}, quizzes:{}, last:1});
  let state = defaults(), storageOK = true, toastTimer, filter = 'all', query = '', timerId, timerDeadline = 0, timerSeconds = 900;
  const validKey = k => /^(f\d{1,2}|p[1-5])_\d+(?:_\d+_\d+)?$/.test(k) || /^(contrib\d{1,2}|calc_(price|clients|variable|fixed))$/.test(k);
  ...
  function fieldHTML(item,index,prefix,n) {
    const [title,body,count]=item, key=prefix+'_'+index;
    if (item.length===2 && body===0) return `<p class="worksheet-hint">${esc(title)}</p>`;
    if (item.length===2) return `<label class="field"><span>${esc(title)}</span><textarea data-answer="${key}" maxlength="20000" rows="${Math.max(3,body)}" placeholder="Registre as descobertas da equipe...">${esc(state.answers[key])}</textarea></label>`;
    const fixed=n===8&&index===0 ? modelFields : n===12&&index===0 ? pitchFields : null;
    return `<h3 class="field-title">${esc(title)}</h3><div class="table-wrap" tabindex="0" role="region" aria-label="Tabela: ${esc(title)}"><table><thead><tr>${body.map(h=>`<th scope="col">${esc(h)}</th>`).join('')}</tr></thead><tbody>${Array.from({length:count},(_,r)=>`<tr>${body.map((h,c)=> fixed&&c===0 ? `<td><span class="row-label">${fixed[r]}</span></td>` : `<td><textarea aria-label="${esc(title)}, linha ${r+1}, ${esc(h)}" data-answer="${key}_${r}_${c}" maxlength="20000" rows="2">${esc(state.answers[key+'_'+r+'_'+c])}</textarea></td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
  }
  ...
  function updateCalc() {
    ...
    const half=Math.floor(vals.clients/2); $('#calc-warning').textContent=`Com ${half} cliente(s), o saldo seria ${money(half*(vals.price-vals.variable)-vals.fixed)}. O que mudaria no projeto?`;
  }
  ...
  document.addEventListener('input',e=>{
    ...
    else if(el.dataset.answer){state.answers[el.dataset.answer]=el.value;save();if(el.dataset.calc)updateCalc();}
  });
  document.addEventListener('change',e=>{const el=e.target;if(el.dataset.quiz){const n=Number(el.dataset.quiz);state.quizzes[n]=Number(el.value);save();$('#quiz-feedback').hidden=false;$('#quiz-feedback').innerHTML=quizMessage(n,Number(el.value));}});
